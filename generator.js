(function () {
  function initSmartLektira() {
    ...
    function createFeedUrl(blogUrl) {
      return (
        blogUrl +
    ...
    function generateOpinion(title) {
      return (
        ...
        title +
        " je pozitivno jer je sadržaj zanimljiv, jasan i potiče razmišljanje. Djelo ostavlja dobar dojam, a posebno je vrijedno zbog poruka koje prenosi čitatelju."
      );
    }
    ...
    function renderTab(tab) {
      ...
      var tabButtons = tabsEl.querySelectorAll(".slg-tab");
    ...
    function tryNextFeed(searchToken, query) {
      if (searchToken !== activeSearchToken) return;

      if (fallbackIndex >= BLOG_URLS.length) {
        cleanupScript();
        setStatus("");
        setMeta([]);
        tabsEl.style.display = "none";
        setOutput("Nijedna varijanta feeda nije uspjela.");
        return;
      }

      currentBlogUrl = BLOG_URLS[fallbackIndex];
      fallbackIndex++;
      ...
      setMeta([
        "Pokušaj: " + currentBlogUrl
      ]);
      setOutput("Učitavanje podataka...");

      currentScript = document.createElement("script");
      currentScript.src = createFeedUrl(currentBlogUrl);
      currentScript.async = true;

      currentScript.onerror = function () {
        tryNextFeed(searchToken, query);
      };
    ...
    function searchPost() {
      ...
      tryNextFeed(activeSearchToken, query);
    }
    ...
    window[CALLBACK_NAME] = function (data) {
      ...
      try {
        ...
        tryNextFeed(activeSearchToken, normalizeText(input.value));
      }
    };
