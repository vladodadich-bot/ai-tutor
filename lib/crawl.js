...
function extractByContainerPattern(html, patterns) {
  for (const pattern of patterns) {
    ...
    if (match && match[1]) {
      const cleaned = normalizeExtractedText(match[1], 16000);
...
function extractMainContent(html) {
  ...
  return normalizeExtractedText(bodyHtml, 16000);
}
...
function extractTextPreview(html, maxWords = 100) {
  ...
  const fallbackText = normalizeExtractedText(bodyHtml, 8000);
...
function isLikelyFilePath(pathname) {
  return /\.(jpg|jpeg|png|gif|webp|svg|pdf|zip|rar|css|js|xml|json|mp4|mp3|avi|woff2?|ttf|eot|ico|rss)$/i.test(pathname);
}

function hasBlockedPath(pathname) {
  return (
...
function hasBlockedSearch(search) {
  return (
...
function isHardBlockedUrl(urlObj) {
  ...
  if (hasBlockedPath(pathname)) return true;
  if (hasBlockedSearch(search)) return true;
...
export function classifyUrl(url) {
  try {
    ...
    if (!segments.length) return 'discovery';
...
export function normalizeUrl(url) {
  try {
    ...
    const isDiscoveryPath = classifyUrl(u.toString()) === 'discovery';
    ...
    u.searchParams.forEach((_, key) => {
      ...
      if (!allowedDiscoveryParams.has(k)) {
        paramsToDelete.push(key);
      }
    });
...
function scoreInternalLink(link, startUrl) {
  ...
  if (kind === 'content') score += 24;
  if (kind === 'discovery') score += 10;

  if (/\/20\d{2}\//.test(href)) score += 14;
  if (href.includes('.html')) score += 10;
  if (href.includes('/p/')) score += 8;
  if (href.includes('updated-max=')) score += 8;
  if (href.includes('max-results=')) score += 6;
  ...
  if (
    text.includes('older posts') ||
    text.includes('older post') ||
    text.includes('stariji postovi') ||
    text.includes('ältere beiträge') ||
    text.includes('weitere beiträge')
  ) {
    score += 14;
  }
...
function extractLinks(html, pageUrl, rootOrigin, startUrl) {
  ...
  while ((match = regex.exec(html)) !== null) {
    ...
    candidate.priority = scoreInternalLink(candidate, startUrl);

    if (!existing || candidate.priority > existing.priority || (candidate.text && !existing.text)) {
...
async function crawlPage(url, rootOrigin, startUrl) {
  ...
  const discoveredLinks = extractLinks(html, finalUrl, rootOrigin, startUrl || finalUrl);
...
function scoreQueueUrl(url, startUrl) {
  ...
  if (href === start) score += 100;
  if (kind === 'content') score += 30;
  if (kind === 'discovery') score += 14;
  if (/\/20\d{2}\//.test(href)) score += 16;
  if (href.includes('.html')) score += 10;
  if (href.includes('/p/')) score += 8;
  if (href.includes('updated-max=')) score += 10;
  if (href.includes('max-results=')) score += 8;
  if (/\/page\/\d+/.test(href)) score += 6;
...
export async function crawlSinglePage(url) {
  ...
  while (queue.length > 0 && crawledPages.length < MAX_PAGES) {
    ...
    try {
      const page = await crawlPage(currentUrl, rootOrigin, startUrl);
...
export async function crawlBatchPages(urls, rootOrigin) {
  ...
  const seen = new Set();
  const startUrl = safeUrls.length ? normalizeUrl(safeUrls[0]) : '';
  ...
  const batchResults = await runWithConcurrency(
    ...
    async (currentUrl) => {
      try {
        const page = await crawlPage(currentUrl, rootOrigin, startUrl || currentUrl);
