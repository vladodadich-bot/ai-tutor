...
function shouldSkipDiscoveryNoise(urlObj, linkText) {
  ...
  if (pathname.startsWith('/search') || pathname.includes('/label/') || pathname.includes('/tag/') || pathname.includes('/category/') || pathname.includes('/author/')) {
    return true;
  }

  if (search.includes('label=') || search.includes('search=') || search.includes('q=')) {
    return true;
  }
