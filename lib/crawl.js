...
function isArchiveLikeUrl(urlObj) {
...
function isBlockedAnchorText(text) {
  ...
  return blockedPhrases.includes(value);
}
...
export function classifyUrl(url) {
  try {
    ...
    if (isArchiveLikeUrl(u)) return 'discovery';
...
export function normalizeUrl(url) {
  try {
    ...
    const keepParams = new Set(['p', 'page_id', 'id']);
    const toDelete = [];
    u.searchParams.forEach((_, key) => {
      const k = String(key || '').toLowerCase();
      if (!keepParams.has(k)) toDelete.push(key);
    });
...
function scoreInternalLink(link) {
  ...
  if (kind === 'content') score += 12;
  if (kind === 'discovery') score -= 8;
...
function extractLinks(html, pageUrl, rootOrigin) {
  ...
  while ((match = regex.exec(html)) !== null) {
    ...
    if (isBlockedAnchorText(rawText)) continue;
    ...
    if (isArchiveLikeUrl(urlObj)) continue;
    ...
    results.push({
      text: rawText,
      href: normalized,
      priority: scoreInternalLink({ text: rawText, href: normalized })
    });
  }
  ...
  if (canonicalHref) {
    ...
    if (normalizedCanonical) {
      try {
        ...
        if (canonicalObj.origin === rootOrigin && !isArchiveLikeUrl(canonicalObj) && !isHardBlockedUrl(canonicalObj)) {
...
export async function crawlSinglePage(url) {
  ...
  while (queue.length > 0 && crawledPages.length < MAX_PAGES) {
    ...
    try {
      ...
      if (String(page.page_kind || '') === 'content') {
        crawledPages.push({
          url: page.url,
