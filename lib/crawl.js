...
function extractTextPreview(html) {
  ...
  if (content) return limitWords(content, 100);

  const bodyHtml = extractBodyHtml(html);
  return limitWords(normalizeExtractedText(bodyHtml, 4000), 100);
}
...
function shouldSkipAnchorText(text) {
  const value = String(text || '').toLowerCase().trim();
  if (!value) return false;

  return (
    value === 'noviji postovi' ||
    value === 'stariji postovi' ||
    value === 'older posts' ||
    value === 'newer posts' ||
    value === 'ältere beiträge' ||
    value === 'neuere beiträge'
  );
}

export function shouldSkipLink(urlObj) {
...
export function classifyUrl(url) {
  try {
    const u = new URL(url);
    return shouldSkipLink(u) ? 'blocked' : 'content';
  } catch {
    return 'blocked';
  }
}
...
function extractLinks(html, pageUrl, rootOrigin) {
  ...
  while ((match = regex.exec(html)) !== null) {
    ...
    if (shouldSkipAnchorText(rawText)) continue;
    ...
    const normalizedHref = normalizeUrl(urlObj.toString());
    if (!normalizedHref) continue;

    results.push({
      text: rawText,
      href: normalizedHref,
      priority: scoreInternalLink({ href: normalizedHref, text: rawText })
    });
  }
...
async function crawlPage(url, rootOrigin) {
  ...
  return {
    ...
    text_preview: extractTextPreview(html),
    content: mainContent ? mainContent.slice(0, 12000) : '',
    page_kind: 'content'
  };
}
...
async function crawlBatchWorker(url, rootOrigin) {
  const normalizedUrl = normalizeUrl(url);
  ...
  if (!normalizedUrl) {
    return {
      ...
      url: String(url || ''),
      error: 'Invalid crawl URL'
    };
  }

  try {
    return await crawlPage(normalizedUrl, rootOrigin);
  } catch (err) {
    return {
      requested_url: normalizedUrl,
      url: normalizedUrl,
...
export async function crawlBatchPages(urls, rootOrigin, concurrency = 6) {
  ...
  async function runner() {
    while (true) {
      ...
      results[index] = await crawlBatchWorker(list[index], rootOrigin);
    }
  }
