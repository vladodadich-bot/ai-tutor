...
function extractByContainerPattern(html, patterns) {
  for (const pattern of patterns) {
    ...
    if (match && match[1]) {
      const cleaned = normalizeExtractedText(match[1], 15000);
...
function extractMainContent(html) {
  ...
  return normalizeExtractedText(bodyHtml, 15000);
}
...
function extractTextPreview(html) {
  ...
  return normalizeExtractedText(bodyHtml, 3000);
}
...
async function crawlPage(url, rootOrigin) {
  ...
  const content = extractMainContent(html);
  const textPreview = content ? content.slice(0, 3000) : extractTextPreview(html);
